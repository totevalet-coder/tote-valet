    ...
    try {
      ...
      if (authData.user) {
        ...
        if (customerError) {
          ...
          // Non-fatal — user is created, profile may need setup
        }
      }
        ...
        <div className="bg-brand-navy px-6 pt-8 pb-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              {step !== 'done' && (
                <p className="text-brand-blue text-xs mt-0.5">Step {stepNum} of 3 — {stepLabels[(stepNum - 1)]}</p>
              )}
            </div>
          </div>
        ...
        <div className="px-8 py-8">
          ...
          {step === 1 && (
            <div className="space-y-4">
              ...
              <button
                onClick={() => {
                  if (!data.name || !data.phone || !data.address || !data.email || !data.password) {
          ...
          {step === 2 && (
            <div className="space-y-4">
              ...
              <div className="bg-brand-blue/5 border-2 border-brand-blue/20 rounded-2xl p-5 text-center">
                ...
                <p className="text-brand-navy font-bold text-sm">Payment Setup via Stripe</p>
                <p className="text-gray-500 text-xs mt-1 leading-relaxed">
                  Secure payment processing will be configured with Stripe. Billing activates on your first pickup.
                </p>
            ...
            <div className="space-y-5">
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-3">
                  How many starting totes?
                </label>
                <div className="grid grid-cols-4 gap-2">
                  {[1, 2, 3, 4].map(n => (
                    <button
                      key={n}
                      onClick={() => update('startingTotes', n)}
                ...
                <p className="text-xs text-gray-400 mt-2">
                  Each tote stored = $15/mo. Empty totes at home = $1/wk each.
                </p>
              </div>

              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1.5">
                  Preferred First Pickup Date
                </label>
                <input
                  type="date"
                  value={data.firstPickupDate}
                  onChange={e => update('firstPickupDate', e.target.value)}
          ...
          {step === 'done' && (
            <div className="text-center py-4">
              ...
              <p className="text-gray-500 text-sm mb-2">
                Welcome to Tote Valet, <strong>{data.name.split(' ')[0]}</strong>!
              </p>
              <p className="text-gray-400 text-xs mb-6">
                A confirmation SMS has been sent to {data.phone}. Your first pickup is scheduled for{' '}
              ...
              <div className="bg-brand-navy/5 rounded-2xl p-4 mb-6 text-left space-y-2">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-semibold text-brand-navy">
                    ${(data.startingTotes * 15).toFixed(2)}/mo
                  </span>
                </div>
              </div>
              <button

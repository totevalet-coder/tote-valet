...
export default function SorterHomePage() {
  ...
  const loadDropZone = useCallback(async () => {
    // Drop zone = totes with status 'picked'
    ...
    // Sorted today = returned_to_station status (staged for route)
  ...
  async function handleToteScan(e: React.FormEvent) {
    ...
    for (const r of routes ?? []) {
      ...
      const hasIt = stops.some(s => s.tote_ids.includes(val))
      if (hasIt) { foundRoute = { id: r.id, driverId: r.driver_id ?? '' }; break }
    }
  ...
  async function handleZoneScan(e: React.FormEvent) {
    ...
    // Zone barcode = route ID (e.g. RT-001)
    ...
    // Move tote to returned_to_station (staged for route)
  ...
  return (
    <div className="px-5 pt-6 pb-6 space-y-5">
      ...
      <div className="card border-2 border-brand-blue/30 bg-brand-blue/5 space-y-3">
        ...
        {destination && scannedTote && !zoneSaved && (
          <div className="space-y-3">
            ...
            <form onSubmit={handleZoneScan} className="flex gap-2">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  placeholder={`Scan ${destination.zoneLabel}...`}
                  className="input-field pl-9 text-sm"
                />
              </div>
              <button type="submit" className="bg-brand-navy text-white rounded-xl px-4 font-semibold text-sm">OK</button>
            ...
            <button
              onClick={() => { setDestination(null); setScannedTote(null); setScanError(''); setZoneError('') }}
      ...
      {/* Link to staging */}
      <button
        onClick={() => router.push('/sorter/staging')}

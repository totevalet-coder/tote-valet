...
export default function LoadVerificationPage() {
  ...
  const load = useCallback(async () => {
    ...
    if (!r) { router.push('/warehouse/sort/staging'); return }
    ...
    const { data: toteData } = await supabase.from('totes').select('id, seal_number, customer_id').in('id', allToteIds)
  ...
  function handleScan(e: React.FormEvent) {
    ...
    if (idx === -1) { setScanError(`${val} is not on this route.`); setScanValue(''); return }
    if (totes[idx].loaded) { setScanError(`${val} already scanned.`); setScanValue(''); return }
  ...
  async function handleConfirm() {
    if (!route) return
    setConfirming(true)
    await supabase.from('totes').update({ status: 'in_transit', last_scan_date: new Date().toISOString() }).in('id', totes.map(t => t.id))
  ...
  if (confirmed) {
    return (
      <div className="px-5 pt-6 pb-6 space-y-5">
        ...
        <button onClick={() => router.push('/warehouse/sort/staging')} className="btn-primary w-full">
  ...
  return (
    <div className="px-5 pt-6 pb-6 space-y-5">
      <button onClick={() => router.push('/warehouse/sort/staging')} className="flex items-center gap-2 text-gray-500 text-sm">
      ...
      <section>
        <h2 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">Totes ({totes.length})</h2>
        <div className="space-y-2">
          {totes.map(t => (
            <div key={t.id} className={`card flex items-center gap-3 ${t.loaded ? '' : 'opacity-60'}`}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${t.loaded ? 'bg-green-100' : 'bg-gray-100'}`}>
                {t.loaded ? <CheckCircle2 className="w-4 h-4 text-green-600" /> : <Package className="w-4 h-4 text-gray-400" />}
              </div>

...
export default function StagingPage() {
  ...
  const loadZones = useCallback(async () => {
    ...
    for (const r of routes as Route[]) {
      ...
      const { data: totes } = await supabase.from('totes').select('id, status, customer_id').in('id', allToteIds)
      const enriched = await Promise.all((totes ?? []).map(async t => {
        ...
        return { id: t.id, customerName: cust?.name ?? 'Unknown', staged: t.status === 'returned_to_station' }
      }))

      zoneList.push({
        routeId: r.id, driverName: driver?.name ?? 'Unassigned', date: r.date,
        totalTotes: allToteIds.length, stagedTotes: enriched.filter(t => t.staged).length, totes: enriched,
      })
    }
          ...
          {zones.map(zone => {
            ...
            return (
              <div key={zone.routeId} className="card space-y-3">
                <button onClick={() => setExpanded(isExpanded ? null : zone.routeId)} className="w-full flex items-center gap-3 text-left">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${fullyStaged ? 'bg-green-100' : 'bg-brand-blue/10'}`}>
                    {fullyStaged ? <CheckCircle2 className="w-5 h-5 text-green-600" /> : <Package className="w-5 h-5 text-brand-blue" />}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="font-bold text-brand-navy text-sm">Zone {zone.routeId}</span>
                      {fullyStaged && <span className="status-pill bg-green-100 text-green-700 text-xs">Ready to Load</span>}
                    </div>
                ...
                <div>
                  ...
                  <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                    <div className={`h-full rounded-full transition-all ${fullyStaged ? 'bg-green-500' : 'bg-brand-blue'}`} style={{ width: `${pct}%` }} />
                  </div>
                </div>
                  ...
                  <div className="pt-2 border-t border-gray-100 space-y-2">
                    {zone.totes.map(t => (
                      <div key={t.id} className={`flex items-center gap-3 ${!t.staged ? 'opacity-60' : ''}`}>
                        <div className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 ${t.staged ? 'bg-green-100' : 'bg-gray-100'}`}>
                          {t.staged ? <CheckCircle2 className="w-3.5 h-3.5 text-green-600" /> : <Package className="w-3.5 h-3.5 text-gray-400" />}
                        </div>
                ...
                {fullyStaged && (
                  <button onClick={() => router.push(`/warehouse/sort/load/${zone.routeId}`)}

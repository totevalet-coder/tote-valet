...
`customers`, `totes`, `bins`, `routes`, `pick_lists`, `errors`, `regions`, `warehouses`, `locations`, `warehouse_rows` (⏳ pending)
...
### Warehouse Floor Map (TODO #10 — ⏳ migration PENDING, not yet run)
...
- Whole **rows are freely draggable** to any position, and each row stores its **own real orientation** (`warehouse_rows.rotation`, 0 or 90 — horizontal/vertical) — not all rows run the same direction on a real floor. This is why a new `warehouse_rows` table exists at all rather than adding columns to `bins`: position/orientation is a property of the row as a unit.
- **Two separate rotation mechanisms, not one**: a global "Rotate View" button (`WarehouseFloorMap`'s own client state, never persisted — lets the operator spin the whole map 90°/180°/270° to match which way they're facing on the floor) and each row's own persisted orientation. Dragging math accounts for the current view rotation (`unrotateDelta` in the component) so dragging still feels natural no matter how the view is currently spun.
...
New table `warehouse_rows` (`warehouse_id`, `row`, `map_x`, `map_y`, `rotation`) + two new nullable columns on `locations` (`map_x`, `map_y`, null = "not yet placed"). Both use integer **grid cells, not pixels** — matches the Excel-drag-to-fill metaphor Bin Layout already uses. New files: `src/lib/warehouseRows.ts` (fetch/upsert), `src/components/admin/WarehouseFloorMap.tsx` (the canvas itself).
Full DDL: `schema.sql`, search "warehouse_rows table + locations.map_x/map_y". **Ask the user whether this has been run yet** — until it has, the Floor Map tab still renders (bins fall back to an in-memory default layout, zones all show as "not yet placed"), but drags/rotations won't actually persist across a reload — they'll silently no-op on save rather than erroring loudly, so a quick live test after running the migration is worth doing before assuming it's fully working.
...
- ⏳ **PENDING** — `warehouse_rows` table + `locations.map_x`/`map_y` (Warehouse Floor Map, TODO #10). Added 2026-08-09, not yet run. See the "Warehouse Floor Map" section above.
...
10. ~~Warehouse Editor~~ ✅ Built 2026-08-09 as **Floor Map**, a new tab in `admin/warehouse-setup` alongside the existing list-based "Bin Layout" tab (kept, not replaced — user's explicit call). Full design + schema in the "Warehouse Floor Map" section below. **⏳ Needs its own migration run** (`warehouse_rows` table + `locations.map_x`/`map_y`) before positions actually persist — see that section. Not yet tested live by the user.

...
### Warehouse Floor Map (TODO #10 — core migration ✅ live 2026-08-09; a follow-up migration ⏳ pending, see below)
...
Full DDL: `schema.sql`, search "warehouse_rows table + locations.map_x/map_y" (✅ applied 2026-08-09, GRANT-verified — 7 rows for `warehouse_rows`/`authenticated`) and "warehouses.map_view_rotation" (⏳ **PENDING as of this writing** — added 2026-08-09, not yet run; falls back to viewing-rotation-0-that-doesn't-persist until it is, same safe-degradation pattern as everything else in this feature). **Ask the user whether the `map_view_rotation` migration has been run before assuming view rotation actually persists.**
...
10. ~~Warehouse Editor~~ ✅ Built and core migration applied 2026-08-09 as **Floor Map**, a new tab in `admin/warehouse-setup` alongside the existing list-based "Bin Layout" tab (kept, not replaced — user's explicit call). Full design + schema in the "Warehouse Floor Map" section below. `warehouse_rows` GRANT verified live. **User tested it live 2026-08-09 and found two real gaps, both fixed same day**: row rotation was only a horizontal/vertical toggle, not the full 4-way needed to match a real floor (fixed); the view-rotation button wasn't persisting across reloads (fixed, but needs its own new migration — `warehouses.map_view_rotation`, ⏳ pending, see the Warehouse Floor Map section above). Drag-and-drop itself confirmed working/persistent by the user. Re-test once that last migration is run before calling this fully confirmed.

...
### Warehouse Floor Map (TODO #10 — ✅ migration applied and GRANT-verified live 2026-08-09)
...
Full DDL: `schema.sql`, search "warehouse_rows table + locations.map_x/map_y". Confirmed live via `information_schema.role_table_grants` on 2026-08-09 (7 rows for `warehouse_rows`/`authenticated`, including `SELECT`/`INSERT`/`UPDATE`/`DELETE`) — safe to build against. Still worth a live end-to-end test (drag a row, rotate it, place a zone, reload the page, confirm it stuck) before treating this as fully working, not just deployed.
...
10. ~~Warehouse Editor~~ ✅ Built and migration applied 2026-08-09 as **Floor Map**, a new tab in `admin/warehouse-setup` alongside the existing list-based "Bin Layout" tab (kept, not replaced — user's explicit call). Full design + schema in the "Warehouse Floor Map" section below. `warehouse_rows` GRANT verified live. Not yet exercised end-to-end by the user (drag a row, rotate it, place a zone, confirm it survives a reload) — worth doing before calling this fully confirmed.
